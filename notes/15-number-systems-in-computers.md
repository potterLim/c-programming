...
이 문서에서는 이진수가 표현할 수 있는 다양한 데이터 유형 중에서 정수(integer)를 중심으로 살펴본다. 컴퓨터가 정수를 어떻게 이진수로 저장하고 해석하는지를 이해함으로써 컴퓨터 내부의 데이터 표현 방식에 대한 기초 개념을 확립하는 것이 목적이다.  
이러한 개념은 이후 다른 과목에서 다루게 될 실수, 문자, 색상, 명령어 등 다양한 데이터가 컴퓨터에서 어떻게 저장되고 처리되는지를 이해하는 데 필요한 기반 지식이 될 것이다.
...
| 진법   | 사용 가능한 숫자 | 예시                           |
| ------ | ---------------- | ------------------------------ |
| 2진법  | 0, 1             | `1011₍₂₎` 또는 `0b1011` = `11` |
| 8진법  | 0 ~ 7            | `75₍₈₎` 또는 `075` = `61`      |
| 10진법 | 0 ~ 9            | `42₍₁₀₎` 또는 `42`             |
| 16진법 | 0 ~ 9, A ~ F     | `1F₍₁₆₎` 또는 `0x1F` = `31`    |
