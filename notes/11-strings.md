...
문자열 리터럴은 읽기 전용이므로 다음과 같은 코드는 위험하다.
...
문자열은 사용자 입력, 메시지 처리, 파일 이름, 오류 로그 등 거의 모든 프로그램에서 사용된다.  
또한 배열, 포인터 개념이 종합적으로 담겨 있어 프로그래밍 실력을 키우는 데 매우 중요하다.

## 정리

- 문자열은 `'\0'`으로 끝나는 `char` 배열이다.
- `scanf()`와 `fgets()`는 입력 방식의 차이와 장단점이 있다.
- 문자열 함수는 항상 버퍼 크기를 고려해야 한다.
- 문자열은 포인터처럼 접근되지만 수정 가능 여부를 잘 구분해야 한다.
- 항상 `'\0'`을 의식하고 문자열을 안전하게 다루자.